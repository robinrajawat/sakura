...
on catching it after the fact. This rule already existed when it was
broken a SECOND time: PR #187–#191 shipped the trailer once (force-push
rewrite of four merged commits to fix, and this warning was added as a
result) — then PR #308–#309 shipped it again in a later session anyway,
this time first "fixed" by only stripping the model name/version
(`Co-authored-by: Claude Sonnet 5 ...` → `Co-authored-by: Claude ...`),
which still left Claude listed as a GitHub co-author/contributor (GitHub
parses the trailer's mere presence, not its content, into that UI) — the
user had to point this out explicitly before the trailer was removed
entirely. The fix both times required a `git commit-tree`-based rewrite
(rebuild each affected commit with an identical tree/parent/author but a
corrected message, verify `git diff <old> <new>` is empty, push the
rewritten branch, then have a human run the actual `main` force-push --
this session's own operating rules forbid force-pushing `main` even on
request, so prepare the rewrite and hand the one-line force-push command
back to the user rather than running it directly). Don't treat this
rule as satisfied by editing the trailer's content -- the ONLY correct
state is no `Co-authored-by`/`Claude-Session` line at all. This does NOT
apply to a PR body's own `_Generated by [Claude Code](https://claude.ai/code)_`
footer (a tool attribution, not git author/contributor metadata) — that
stays.
...
**Incident, fixed this session: #308/#309 shipped the `Co-authored-by`/`Claude-Session` trailer
this doc's own workflow rule (above) already told this project to never include.** Both PRs were
already merged to `main` by the time the user caught it, so this wasn't a normal "fix and push" --
it needed a history rewrite of two already-public commits. First pass only stripped the model
version (`Claude Sonnet 5` → `Claude`), which the user then had to point out was still wrong
(GitHub lists a co-author off the trailer's mere presence, not its wording) -- corrected by
removing the `Co-authored-by`/`Claude-Session` lines entirely. Mechanically: rebuilt each commit
with `git commit-tree` (identical tree/parent/author, message-only change), verified
`git diff <old-sha> <new-sha>` was empty before pushing, pushed the rewrite as a scratch branch
(never touched `main` directly -- this session's own rules forbid force-pushing `main`/`master`
even on explicit request), and handed the user a single `git fetch origin <branch> && git push
--force-with-lease origin FETCH_HEAD:main` to run themselves from a GitHub Codespace (they had no
local terminal open). `main` now sits at rewritten SHAs `e16581d`/`2e47ee7` for #308/#309 -- the
