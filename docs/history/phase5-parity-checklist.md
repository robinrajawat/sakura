...
| Presenter Mode | ⚠️ | Slide grouping, Prev/Next/arrow-keys (Phase 3), plus timer, blackout, laser pointer, overview grid, closing slide, and a floating Notes/Q&A panel (§6.6) — no Whiteboard mirroring or Audience View/dual-screen yet, but real progress on both: presenting state now lives in a real `usePresenterStore.ts` (not local `useState`), and `?sakuraAudience=1` now boots a chromeless `AudienceWindow.tsx` showing the real active document via `PresenterMode.tsx` — reachable only by navigating directly to that query param so far, not yet opened or driven by another window. Still needed: a `window`-exposed cross-window bridge, the opener actually calling `window.open()`, and Diagrams gaining a real `isWhiteboard` concept for mirroring. See phase6-full-parity-plan.md's §6.6 section for the full scoping and translation plan |
