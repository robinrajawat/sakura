...
| Word export | ⚠️ | Real .docx via the `docx` library (Phase 3); heading styles, a real TOC field, a branding footer, and note-image embedding now built (§6.6, real OOXML media parts via `docx`'s `ImageRun`, png/jpg/gif/bmp only); no Decision Log cards, rich formatting, tables |
