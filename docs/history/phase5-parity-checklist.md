...
| Decision Log | ⚠️ | Rebuilt to its real node-anchored schema (one per node, 5 structured fields, status, author, §6.7 #224) with live-editor own-node + collapsed-subtree badge dots (§6.7 #225), a real anchor-picker popover (§6.7), and Preview/PDF/Word/PowerPoint card rendering (§6.7); still no Excel export |
...
| Pad — Decision Log | ⚠️ | Real node-linking + structured fields (context/decision/rationale/alternatives/impact) + author/status built (§6.7 #224), outline badge dots (§6.7 #225), a real anchor-picker popover for re-anchoring (§6.7, `components/AnchorPicker.tsx`), and Preview/PDF/Word/PowerPoint card rendering (§6.7); no Excel export |
...
| Excel (Decision Log .xlsx) export | ❌ | Not started -- no longer blocked (Decision Log is now a real feature, §6.7), just not yet attempted. Legacy's real Excel export is Decision-Log-specific, not a general outline export, and needs a new SheetJS/`xlsx` dependency this project doesn't have yet |
