...
| PWA install | ⚠️ | Manifest + service worker exist (Phase 3); title-bar theme-color now syncs live on every theme change (§6.11) and the maskable icon was verified byte-identical to legacy's own real one with correct safe-zone padding (§6.11 — the previous "single icon set, no maskable-variant distinction" note here was stale) — service worker is still runtime cache-first, not legacy's real static precache strategy |
