...
| Deep theming | ⚠️ | Light/Dark toggle (Phase 3), accent color, System auto-theme, and node-text color presets, all persisted across sessions (§6.7) — no Chrome background presets (investigated (§6.7): confirmed unreachable in legacy's own UI, not a gap in this port). §6.11's visual-token pass also fixed a real remaining drift: `NodeText.tsx`'s semantic-markup colors (`[Section]`/code/alert/note/link styling) were baked in as light-theme-only hex instead of reading the live per-theme CSS custom properties — now fixed, see the Semantic markup colors row below |
| PWA install | ✅ | Manifest + service worker exist (Phase 3); title-bar theme-color now syncs live on every theme change (§6.11), the maskable icon was verified byte-identical to legacy's own real one with correct safe-zone padding (§6.11 — the previous "single icon set, no maskable-variant distinction" note here was stale), and the service worker now uses a real static precache strategy matching legacy's own (§6.11) — see the Installing as an App section below |
...
| Account sign-in + sync | ⚠️ | Google AND email/password sign-in (§6.8) + bidirectional Firestore doc sync exist (Phase 4), wired to the real production project. Real debounced autosave now built too (§6.8): an outline edit queues a push 1500ms after edits settle, matching legacy's own real `queueSync`/`flushSyncQueue` timer exactly, plus a sync-status indicator (Syncing…/Synced/error text) replacing the old manual "Push to cloud" button. Full sharing/collaboration now built too (§6.8, see the Sharing row below), and now the real persistent top-bar sync-status dot too (§6.8, see the Sync health status-bar dot row below). Full whole-app JSON export/import and "Undo last restore" are also now built (§6.8, see the Data & Backup section below) |
| Version History | ⚠️ | Document outline core built (§6.8, `store/versionHistoryStore.ts`) — same real IndexedDB scheme/constants as legacy (`docrev:<id>`, 10min auto-capture gap, 20-version cap, `'Auto'`/`'Manual checkpoint'`/`'Before restoring an older version'` reasons), a real History panel (Save a version now / Restore), restoring is non-destructive. Deliberately scoped to nodes/title only, active document only — no per-node view, no Pad/diagrams/Q&A/decision-log/attachment/remark capture (Pad isn't wired into documentsStore's save cycle at all yet), no background-document restore, and no Hub-domain histories (To-Dos/Meeting Notes/Journal/Library each have their own separate real mechanism in legacy) |
...
| Semantic markup colors track live theme | ✅ | Fixed §6.11's visual-token pass: `NodeText.tsx` (renders `[Section]`/`` `code` ``/`!alert`/`(note)`/`[[links]]` inline styling) baked in light-theme-only hex values, a real dark-mode legibility bug since legacy's own `--sem-section`/`--sem-alert`/`--sem-code`/`--muted`/`--accent` genuinely differ per theme. Now reads the live CSS custom properties `themeStore.ts` already sets on `document.body`, matching legacy's own CSS exactly; verified both themes' computed colors match `THEME_TOKENS` exactly in real headless Chrome. Same pass also found and fixed `OutlineTree.tsx`'s empty-node placeholder using a hardcoded `#bbb` instead of the `t.mutedText` its sibling call sites (`PresenterSlideView.tsx`/`PreviewPane.tsx`) already used correctly |

## Installing as an App (PWA)

| Feature | Status | Note |
|---|---|---|
| Manifest + service worker | ✅ | Phase 3 build, real static precache strategy built §6.11: `scripts/generate-sw-precache.mjs` reads Vite's own build manifest (`build.manifest: true`) after every build and rewrites `dist/sw.js`'s precache list with the real hashed-filename list (the app shell, PWA manifest, 3 icon files, plus every real hashed JS/CSS/asset) -- direct port of legacy's real `sw.js` strategy (network-first navigation with a cached-fallback, cache-first static assets, everything else passed straight through) adapted for Vite's per-build content-hashed filenames, which legacy's own single unhashed `index.html` never had to deal with. `CACHE_NAME` gets a content-hash suffix automatically every build (unlike legacy's own real manual-bump convention) so `activate()` reliably evicts each previous deploy's now-orphaned cache. Verified end-to-end against a real `vite preview` server in real headless Chrome: the service worker reaches the active state, the real cache holds all 8 expected precache URLs including the real hashed JS bundle, and reloading with the browser context set fully offline still shows the app shell served from that cache |
| Install icon transparency / maskable variant | ✅ | Verified §6.11: `icon-512-maskable.png` is byte-identical to legacy's own real file (`md5sum`-confirmed), correctly declared `"purpose": "maskable"` in `manifest.json`, opaque (no transparency -- correct for a maskable icon, since the OS applies its own background/mask), and visually inspected to have proper safe-zone padding |
| Title bar color follows live theme | ✅ | Built §6.11: `themeStore.ts`'s new `syncThemeColorMeta`, direct port of legacy's real `applyChromeColors()` theme-color write, called from the shared `applyCssVariables` on both `init()` and every `setTheme()` -- `<meta name="theme-color">` now tracks light/dark live instead of staying pinned to `index.html`'s static initial value forever |
...
| 1. Local safety copy (IndexedDB mirror) | ✅ | Built §6.8: direct port of legacy's real `mirrorToIndexedDb`/`updateSafetyCopyStatus`/`restoreFromIndexedDbMirror`, same real IndexedDB database/store/key names and 1200ms debounce (`store/backupStore.ts`). "Restore…" button under Settings → Data & Backup. Debounce is wired to outline edits only (the highest-value, highest-frequency edit surface — `web/`'s multi-store architecture has no single "anything changed" event the way legacy's one script does). Now snapshots a `preRestoreSnapshot` before restoring too (§6.8, once "Undo last restore" itself landed) |
| 2. Auto-backup to file (File System Access API) | ✅ | Built §6.8: direct port of legacy's real Connect…/Disconnect…/Reconnect state machine (`store/fsBackupStore.ts`), driven off the same 1200ms debounce as tier 1. Chrome/Edge only |
| 3. Export/Import (whole-app JSON) | ✅ | Built §6.8: direct port of legacy's real `exportAllData`/`importAllDataFromFile`/`importAllDataFromPayload` (`store/dataIoStore.ts`, `components/DataIoSettings.tsx`) — same real envelope shape as tiers 1/2, same loose shape validation, same clear-and-rewrite-with-rollback-on-failure mechanics. No `preserveKeys`/secret-preservation logic (legacy's own real file-import call site already passes none) and no `SAKURA_JUST_RESTORED_KEY` cloud-pull-and-merge guard (no equivalent mechanism exists in `web/`'s architecture) |
| Undo last restore | ✅ | Built §6.8: direct port of legacy's real `restoreFromPreRestoreSnapshot`, one level deep (matches legacy's own real behavior verbatim). Works after either a whole-app import OR a tier-1 safety-copy restore — both write the same shared `PRE_RESTORE_SNAPSHOT_KEY` |
