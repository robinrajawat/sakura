...
| Presenter Mode | ⚠️ | Slide grouping, Prev/Next/arrow-keys (Phase 3), plus timer, blackout, laser pointer, overview grid, closing slide, and a floating Notes/Q&A panel (§6.6) — no Whiteboard mirroring or Audience View/dual-screen. Re-investigated (§6.6): NOT actually blocked on client-side routing (a corrected prior claim) — legacy's real mechanism needs only a query-param boot check, no path routing; the real blockers are `PresenterMode.tsx`'s presenting state living in local `useState` instead of a shared store, and Diagrams having no `isWhiteboard` concept yet. See phase6-full-parity-plan.md's §6.6 section for the full corrected scoping and translation plan |
