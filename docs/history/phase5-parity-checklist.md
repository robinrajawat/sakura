...
| Presenter Mode | ⚠️ | Slide grouping, Prev/Next/arrow-keys (Phase 3), plus timer, blackout, laser pointer, overview grid, closing slide, and a floating Notes/Q&A panel (§6.6) — no Whiteboard mirroring or Audience View/dual-screen. Re-investigated (§6.6): NOT actually blocked on client-side routing (a corrected prior claim) — legacy's real mechanism needs only a query-param boot check, no path routing. Step 1 of the concrete translation plan now landed: presenting state moved from local `useState` into a real `usePresenterStore.ts` (a pure refactor, verified behavior-identical) — still needed before Audience View itself works: the query-param boot check, a `window`-exposed cross-window bridge, and Diagrams gaining a real `isWhiteboard` concept. See phase6-full-parity-plan.md's §6.6 section for the full scoping and translation plan |
