...
| Journal | ⚠️ | One-entry-per-date editing, mood, rich text (bullet/numbered list + Ctrl/Cmd+B/I), and a calendar-popover date picker all built (§6.5, pending PR -- see docs/phase6-full-parity-plan.md); no AI rewrite, PDF export, Version History, or search (legacy's own Journal search lives only in the shared Quick Assist / hub-wide search bar, which doesn't exist in web/ yet). Tags UI is correctly NOT a gap: legacy itself has no tags UI for Journal despite README.md referencing them -- a pre-existing doc/code mismatch |
| Library | ⚠️ | Persistence, title/url/urlLabel/tags/favorites, search, tag filtering, and rich text (bullet/numbered list + Ctrl/Cmd+B/I, matching Journal's own narrower toolset) all built (§6.5, pending PR -- see docs/phase6-full-parity-plan.md); no AI rewrite, images, Version History, PDF export, or Quick Assist/Global Search visibility (Quick Assist doesn't exist in web/ yet) |
