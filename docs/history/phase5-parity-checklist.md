...
| Layout controls (tree lines, depth guides, row style, compact rows, text size, indent width, limit reading width) | ⚠️ | Partial, most of it now real (§6.7/§6.10): indent width/hide-tree-lines/outline-numbering (export-only) plus the newly-landed live-editor "Layout" section -- compact rows, text size, limit reading width (a real legacy setting not previously listed here), and row style (which turned out to be real under a different name, `rowHighlightStyle` -- "collapse depth" turned out NOT to be a real legacy feature at all, removed from this row's own name) -- are all real, persisted, adjustable prefs, verified end-to-end in real headless Chrome. Still not built: tree-line connectors and depth guide lines in the LIVE editor specifically (legacy's live tree uses literal monospace ASCII-connector text via `buildPrefix`, already ported to `web/` for exports only -- porting it to the live tree too is real, separately-scoped follow-up work, not just a toggle) |
