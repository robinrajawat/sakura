...
**`framework-migration-plan.md`** carries the actual decision at the top of the file — read that
first if you're touching anything related to `web/`. Short version: `web/` (the React rewrite)
will not be developed further and will never be deployed; `legacy/` is production permanently.
`phase6-full-parity-plan.md`, `phase7-app-shell-and-dashboard-plan.md`,
`phase8-design-system-parity-plan.md`, and `post-cutover-backlog.md` (below) are now historical
records of that stopped effort, not active work queues — kept for reference, not maintained
further.
