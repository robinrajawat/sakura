/**
 * Pure open-tab ordering/cycling logic — a Phase 2 (docs/architecture-plan.md) slice, revisiting
 * "tab state" after it was originally set aside alongside outline search and diagram-anchor
 * state as "core-outline-coupled." That framing doesn't actually hold for these two functions
 * specifically: `cycleOpenTab`/`reorderTab` only ever read/write `openTabs`/`activeTabDocId` —
 * neither touches the outline `nodes` array at all. Same shape of over-broad original judgment
 * as `nodeSearch.ts`'s revisit of search-matching.
 *
