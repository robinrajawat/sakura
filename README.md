...
- **Note** — every node can hold a plain-text note (AI Rewrite/Summarise still work directly on that text — neither needs formatting to operate). A quick, casual place for a comment or aside on a node; for a real rich-text document use the Pad's Notepad below instead. Floating and draggable, with a compact popover view and a full-screen mode. Shows a Backlinks section for any node that `@mentions` it, plus created/last-modified timestamps. Open via toolbar, right-click → More → Note, or `Ctrl/Cmd+Shift+N`.
...
- **Pad** — a whole-document workspace, separate from per-node Notes, with seven tabs:
  - **Notepad** — a rich-text scratchpad (bold/italic/underline/strike, bullet/numbered lists, links, tables, pasted images), the one place in Sakura for a real formatted document rather than a plain note, plus its own **Quote** button for citation-style blocks (select text to wrap it, or insert blank).
...
- **Word export** (`.docx`) — real heading styles, a proper Table of Contents (headings and section markers, with page references — press Ctrl/Cmd+A then F9 after opening to load real page numbers, since Word only caches "1" until fields are recalculated), and Decision Log entries as bordered, status-colored cards. Diagrams-tab pictures embed as real, correctly-scaled pictures.
...
`www.sakura-notes.com` is published by `.github/workflows/deploy.yml`: every push to `main` runs `npm run build -w sakura-legacy` (a plain `vite build` — static assets like `sw.js`, both PWA manifests, and icons live in `legacy/public/`, Vite's own static-passthrough convention, so no custom copy step is needed) and publishes the result via GitHub Actions' native Pages deployment. There's no separate `dist/` branch or manual publish step; `legacy/index.html`/`legacy/hub.html` in the repo are the real source, and CI builds and serves them on every merge. This app coexists in the repo as an npm workspace alongside a React rewrite (`web/`) that was explored and then discontinued (2026-08-31, never deployed) — see `docs/history/web-migration/framework-migration-plan.md` for that decision; this file (`legacy/`) remains production permanently. See `docs/history/architecture-plan.md`'s "Deployment mechanism" and "Repo hygiene" sections for the full history of how this was verified before being switched on.
