...
On a narrow screen (below 768px), Sakura switches to **Mobile Hub mode** automatically: the sidebar, document tabs, and editor step aside and whichever Hub tab you last had open takes over the full screen, maximized. The app bar (Hub switcher, Import/Export, Account/Settings/Help) stays reachable, and you can still switch between To-Dos/Meetings/Journal/Library/Recap from the tab strip inside the panel — but there's no way to close the panel back to the (hidden) outliner short of widening the window past 768px or reloading, since nothing would be left to show underneath it. Nothing about this is a setting — it's a live viewport check, not a persisted mode, so a phone always lands here and a resized desktop window reverts the instant you cross back over the breakpoint.
...
- Below 768px, the outliner (sidebar, document tabs, editor) isn't reachable at all — see Mobile Hub mode above. This is an intentional narrow-viewport tradeoff, not a bug, but it means there's currently no way to edit a document from a phone-sized screen.
