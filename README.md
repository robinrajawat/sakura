...
- Companion panels per node or per document: rich-text Notes, Code blocks, a Decision Log, a whole-document Pad (with Notepad, Q&A, Diagrams, Mind Map, and Files tabs)
...
- **Pad** — a whole-document workspace, separate from per-node Notes, with six tabs:
  - **Notepad** — a rich-text scratchpad with the same toolbar Note uses (minus subtree-summarise, since there's no subtree at the document level).
